    ...
    <main class="pt-90">
        ...
        <section class="shop-checkout container">
            ...
            <div class="shopping-cart">
                @if ($items->count() > 0)
                    <div class="cart-table__wrapper" style="display: flex; flex-direction: column; gap: 16px;">
                            ...
                            <tbody>
                                @foreach ($items as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <form method="POST"
                                                action="{{ route('cart.remove-item', ['rowId' => $item->rowId]) }}">
                                                @csrf
                                                @method('DELETE')
                        ...
                        @else
                            <div>
                                <form action="{{ route('cart.coupon-remove') }}" method="POST"
                                    class="position-relative bg-body" style="max-width: 300px;">
                                    @csrf
                                    @method('DELETE')
                                    <input class="form-control" type="text" name="coupon_code" placeholder="Coupon Code"
                                        value="@if (Session::has('coupon')) {{ Session::get('coupon')['code'] }} Applied! @endif">
                        ...
                        <form action="{{ route('cart.empty-item') }}" method="post">
                            @csrf
                            @method('DELETE')
